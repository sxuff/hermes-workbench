...
Hermes Workbench is a local, single-user interface to your own Hermes install. It is served by the Hermes dashboard, which binds to `127.0.0.1` by default. It is not designed for public hosting. It is a community plugin, not a Nous Research product.
...
- **Build.** `hermes workbench build` reads the desktop sources from your Hermes checkout and never writes to it. It runs `npm install --ignore-scripts`, guided by Hermes's own `package-lock.json`, in a separate workspace under `~/.hermes/hermes-workbench/desktop-build`, then runs Vite and syncs the result into the plugin's own directory. It writes nowhere else. The build runs on the first `hermes workbench` and again after each Hermes update, and needs npm registry access whenever Hermes changes its lockfile.
